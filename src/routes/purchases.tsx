...
import { Plus, FileDown, Pencil, Ban, Download } from "lucide-react";
...
function PurchasesPage() {
  ...
  const fields: FieldDef[] = [
    { name: "no", label: "Bill No", required: true, half: true },
    { name: "date", label: "Date", type: "date", required: true, half: true },
    { name: "party", label: "Supplier", type: "select", required: true,
  ...
  function handleSubmit(v: Record<string, unknown>) {
    ...
    if (editing) { update("purchaseInvoices", editing.id, data); toast.success(`${editing.no} updated`); }
    else { add("purchaseInvoices", { id: newId("pi"), ...data }); toast.success(`${data.no} added`); }
    setEditing(null);
  }

  function downloadBill(i: CInvoice) {
    generateDocPdf({
      type: "Purchase Bill", no: i.no, date: i.date, party: i.party,
      rows: [{ label: "Status", value: i.status }],
      lines: {
        head: ["Description", "Taxable", "GST", "Total"],
        body: [["Material purchase", inr(i.amount - i.gst), inr(i.gst), inr(i.amount)]],
      },
      totals: [{ label: "Grand Total", value: inr(i.amount) }],
  ...
  function exportPdf() {
    generatePdf({
      ...
      subtitle: "Cancelled bills excluded",
      filename: `purchases-${Date.now()}.pdf`,
      head: ["Bill", "Date", "Supplier", "Taxable", "GST", "Total", "Status"],
      body: live.map((i) => [i.no, i.date, i.party, inr(i.amount - i.gst), inr(i.gst), inr(i.amount), i.status]),
      totals: [{ label: "Total purchases", value: inr(live.reduce((a, i) => a + i.amount, 0)) }],
    });
  }
  ...
  return (
    <div>
      <PageHeader title="Purchases"
        ...
        actions={
          <>
            ...
            <Button size="sm" onClick={() => { setEditing(null); setOpen(true); }}><Plus className="mr-1 h-4 w-4" />New purchase</Button>
          </>
        } />
      <div className="p-6">
        <ListShell toolbar={<p className="text-sm font-medium">{live.length} active bills</p>}>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Bill No</TableHead><TableHead>Date</TableHead>
              ...
              {invoices.map((i) => (
                <TableRow key={i.id} className={i.cancelled ? "opacity-60" : ""}>
                  ...
                  <TableCell className="text-right whitespace-nowrap">
                    <Button variant="ghost" size="sm" onClick={() => downloadBill(i)}><FileDown className="h-3.5 w-3.5" /></Button>
      ...
      <EntityDialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setEditing(null); }}
        title="Purchase Bill" fields={fields} mode={editing ? "edit" : "create"}
        initial={editing ?? { date: new Date().toISOString().slice(0, 10), status: "Unpaid", no: `PUR-${Date.now().toString().slice(-6)}` }}
        onSubmit={handleSubmit} />
      <CancelDialog open={!!cancelTarget} onOpenChange={(v) => !v && setCancelTarget(null)}
        title={cancelTarget ? `Cancel ${cancelTarget.no}` : "Cancel"}
        onConfirm={(remark) => { if (cancelTarget) { cancel("purchaseInvoices", cancelTarget.id, remark); toast.warning(`${cancelTarget.no} cancelled`, { description: remark }); } }} />
    </div>
  );
}
